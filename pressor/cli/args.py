...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--format-conversion", action="store_true", help="Convert supported audio inputs to the requested target format without applying Pressor's optimization pipeline.")
    parser.add_argument("--target-format", choices=["ogg", "opus"], default="ogg", help="Target format used by --format-conversion. Default: ogg.")
    parser.add_argument("--conversion-bitrate", default="96k", help="Audio bitrate used by --format-conversion. Default: 96k.")
    parser.add_argument("--convert-lossy-to-ogg", action="store_true", help="Legacy alias for --format-conversion --target-format ogg. Kept for compatibility.")
    parser.add_argument("--ogg-bitrate", default=None, help="Legacy bitrate override for --convert-lossy-to-ogg. Prefer --conversion-bitrate.")
    parser.add_argument("--output-format", choices=["profile", "opus", "ogg"], default="profile", help="Override profile containers for normal encoding. Use opus for .opus output or ogg for .ogg output. Default: profile.")
